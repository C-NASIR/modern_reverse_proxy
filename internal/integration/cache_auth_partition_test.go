...
func TestCacheAuthPartition(t *testing.T) {
	...
	resp, body := sendAuthRequest(t, client, proxyServer.URL, "example.local", "Bearer A")
	if resp.StatusCode != http.StatusOK || string(body) != "Bearer A" {
		t.Fatalf("unexpected response for A: %d %q", resp.StatusCode, string(body))
	}

	resp, body = sendAuthRequest(t, client, proxyServer.URL, "example.local", "Bearer B")
	if resp.StatusCode != http.StatusOK || string(body) != "Bearer B" {
		t.Fatalf("unexpected response for B: %d %q", resp.StatusCode, string(body))
	}

	resp, body = sendAuthRequest(t, client, proxyServer.URL, "example.local", "Bearer A")
	if resp.StatusCode != http.StatusOK || string(body) != "Bearer A" {
		t.Fatalf("unexpected response for A repeat: %d %q", resp.StatusCode, string(body))
	}
...
func sendAuthRequest(t *testing.T, client *http.Client, baseURL, host, auth string) (*http.Response, []byte) {
	t.Helper()
