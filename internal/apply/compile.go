...
func (m *Manager) compile(ctx context.Context, providers []provider.Provider, reg *registry.Registry, breakerReg *breaker.Registry, outlierReg *outlier.Registry, trafficReg *traffic.Registry) (*runtime.Snapshot, *config.Config, error) {
	...
	go func() {
		...
		resultCh <- compileResult{snapshot: snapshot, cfg: resolvedCfg, err: err}
	}()
	...
	case result := <-resultCh:
		return result.snapshot, result.cfg, result.err
	}
}
